...
watch(() => progressWidth.value, (newValue) => {
  const progressbarWidth = progressbar.value.clientWidth;
  const currentbarWidth = progressbarWidth * newValue.replace('%', '') * 0.01;
  console.log(progressbarWidth, currentbarWidth)
  if (progressbarWidth <= currentbarWidth + tooltip.value.clientWidth / 2) {
    console.log('넘는다')
    tooltipLeft = progressbarWidth - (currentbarWidth + tooltip.value.clientWidth);
    arrowRight = '6px';
    ...
    console.log('아니다')
    tooltipLeft = -tooltip.value.clientWidth / 2;
    arrowRight = '50%';
  }
});
    ...
    <div class="progress-bar_container">
      <div class="progress-bar" ref="progressbar">
        <div class="current-bar" ref="currentbar" :style="{ '--progress-width': progressWidth }">
          <span class="tooltip_container" ref="tooltip" :style="{transform: `translateX(${tooltipLeft}px)`, '--arrow-right': arrowRight}">
            {{ activePrice }}
          </span>
        </div>
      </div>
